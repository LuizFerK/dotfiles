...
monitor_res=$(( $monitor_res * $physical_monitor_size))
...
swww img ${wall_dir}/${wall_selection} --transition-type wipe --transition-angle 30 --transition-duration 1 --transition-step 150 & wallust run ${wall_dir}/${wall_selection} && pkill -SIGUSR2 waybar
